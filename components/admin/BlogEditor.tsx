...
import { addBlog, updateBlog, deleteBlog } from './actions';
...
function BlogForm({ blog, onFinished }: { blog?: any; onFinished: () => void }) {
  const { toast } = useToast();
  const form = useForm<BlogFormValues>({
    resolver: zodResolver(formSchema),
    defaultValues: blog ? {
        ...blog,
        expires_at: blog.expires_at ? new Date(blog.expires_at).toISOString().split('T')[0] : '',
  ...
  return (
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        <h2 className="text-2xl font-semibold mb-6">{isEditMode ? 'Edit Blog Post' : 'Add New Blog Post'}</h2>
        ...
        <Button type="submit" disabled={form.formState.isSubmitting}>{isEditMode ? 'Update' : 'Add'} Blog Post</Button>
...
// --- Blog List ---
function BlogList({ blogs, onEdit }: { blogs: any[], onEdit: (blog: any) => void }) {
    const { toast } = useToast();
...
export function BlogEditor({ blogs }: { blogs: any[] }) {
    ...
    return (
        <div className="grid md:grid-cols-2 gap-12">
            <div>
                {editingBlog ? (
                    <BlogForm blog={editingBlog} onFinished={handleFinished} />
                ) : (
                    <BlogForm onFinished={handleFinished} />
                )}
            </div>
            <div>
