...
import { addDirector, updateDirector } from './people-actions';
...
export default function DirectorForm({ director, onFinished }: { director?: any; onFinished: () => void }) {
  ...
  return (
    <FormProvider {...form}>
      <Form {...form}>
          <h3 className="text-lg font-semibold mb-4">{isEditMode ? 'Edit Director' : 'Add New Director'}</h3>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          <FormField control={form.control} name="name" render={({ field }) => (
            <FormItem><FormLabel>Name</FormLabel><FormControl><Input placeholder="Director's Name" {...field} /></FormControl><FormMessage /></FormItem>
          )} />
          <FormField control={form.control} name="position" render={({ field }) => (
            <FormItem><FormLabel>Position</FormLabel><FormControl><Input placeholder="Director's Position" {...field} /></FormControl><FormMessage /></FormItem>
          )} />
          ...
          <div className="flex gap-4">
            <Button type="submit" disabled={form.formState.isSubmitting}>
                {form.formState.isSubmitting ? (isEditMode ? 'Updating...' : 'Adding...') : (isEditMode ? 'Update Director' : 'Add Director')}
            </Button>
